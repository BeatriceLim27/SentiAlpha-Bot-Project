...
class StrategyRunner:
    def __init__(self, model_path, feature_path, confidence_threshold=0.7, respect_sentiment=True, vol_exit_multiplier=1.2):
        ...
        self.df = self.df[(self.df.index >= "2025-03-01") & (self.df.index < "2025-04-01")].copy()
        print(f"[DEBUG] Filtered row count (Mar 2025): {len(self.df)}")
    ...
    def run(self):
        ...
        out_file = os.path.join("backtests", "backtest_results_Mar2025.csv")
        ...
        print(f"[✅] Strategy PnL (Mar 2025): {final_pnl:.2f} USDT")

        plt.figure(figsize=(12, 6))
        plt.plot(self.df["cumulative_pnl"], linewidth=2)
        plt.title("Cumulative PnL (Mar 2025)")
        plt.xlabel("Time")
...
if __name__ == "__main__":
    runner = StrategyRunner(
        ...
        confidence_threshold=0.35,  # fixed threshold
